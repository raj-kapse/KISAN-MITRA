...
| **Siyal Kambale** | Frontend & Deployment | [@Siyalkamble](https://github.com/Siyalkamble) |
| **Naveen Thakur** | Full-Stack Lead & PWA | [@nav548777](https://github.com/nav548777) |
...
This project was developed with extensive assistance from AI coding tools (e.g., LLM-based coding assistants). Specifically:

- **Code generation**:

  - Most of the initial boilerplate and scaffolding (Express server setup, basic routes, PWA configuration, etc.) was generated by AI based on our high-level requirements.
  - Core application logic — including the provider failover chain (Groq → Gemini), rate limiting, device-scoped history, and integration between frontend and backend — was iteratively refined, tested, and debugged by the team with AI assistance.
- **Debugging & refactoring**:

  - We used AI to interpret error messages, suggest fixes, and refactor code for clarity and performance.
- **Documentation & PPT**:

  - README content, slide text, and explanatory prose were drafted with AI assistance and then reviewed and edited by the team to match our actual implementation.
- **Product feature (not development assistance)**:

  - The app’s core functionality relies on calling external LLM APIs (Groq hosting Qwen3.8-27B and Google Gemini) as its AI engine for diagnosis and chat. This is the product’s intended AI capability, not a development-time shortcut.

While AI tools significantly accelerated development, the team is responsible for the final architecture, integration decisions, and behavior of the system, and can explain all critical components in detail.
